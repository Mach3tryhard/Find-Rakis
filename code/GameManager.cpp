    ...
    while (const std::optional event = window.pollEvent()) {
        ...
        if (inMenu) {
            ...
            if (event->is<sf::Event::Resized>()) {
                ...
                sf::Vector2f newCenter = {static_cast<float>(player.getPhysics().getPosition().x),
                                          static_cast<float>(player.getPhysics().getPosition().y)};
...
void GameManager::UpdateGameplay(sf::Time dt) {
...
void GameManager::RenderOverlays() {
    if (player.getWon()) {
